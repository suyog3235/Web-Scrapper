...
def facebook():
    ...
    # facebook login data
    USERNAME = 'Youremailid@gmail.com // username'
    ...
    driver = webdriver.Chrome(options=chrome_options, executable_path=r"CHROME DRIVER PATH")
...
def linkdin():
    ...
    USERNAME = 'youremailid@gmail.com // your username'
    PASSWORD = 'your password'
    ...
    driver.get("https://www.linkedin.com/company/cloud-counselage/posts/?feedView=all")
    time.sleep(5)
    ...
    try:
        for post in total_posts:
            day_time = post.find_all("span", attrs={"class": "visually-hidden"})
            for d in day_time:
                if d.text == "2 months ago":
                    total_posts_in_july += 1

                    likes = post.find_all("span", attrs={'class': "v-align-middle social-details-social-counts__reactions-count"})
                    for like in likes:
                        total_likes += int(like.text)

                    comments = post.find_all("", attrs={"class": "social-details-social-counts__comments"})
                    for comment in comments:
